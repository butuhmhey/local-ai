...
**Last Updated:** 2026-09-08 (Structural design overhaul — typography hierarchy, border-radius tightening, hazy shadows removed)  
**Current Phase:** Phase 3 ✅ COMPLETE + Visual Rebrand ✅ + Chat Verified ✅ + Download-First Gate ✅ + **Visual Overhaul ✅** + **Anti-Vibe Pass ✅** + **Structural Redesign ✅** + **LIVE ✅**  
...
| Time | Change | Files |
|------|--------|-------|
| 2026-09-08 | **STRUCTURAL DESIGN OVERHAUL**: Cut border-radius scale from 8/12/16/20/28px to 4/6/8/10/14px (sharper, not bubbly). Removed hazy `shadow-sm` default on all `.card` surfaces — structure from crisp borders, shadows only on hover/elevation. Removed `shadow-xs` from chat input + sidebar edge shadow. Added Inter weight 800 to Google Fonts. Typographic hierarchy: hero + welcome titles 800 weight / -0.04em tracking / 1.06 line-height; page-header titles 800 / -0.03em; feature card titles bumped 700. Added `landing-eyebrow` badge (uppercase, accent, pill shape) above landing hero title — real hierarchy. Removed dead `.landing-glow` HTML element. Welcome feature cards: left-aligned with `border-top: 2px solid accent` accent strip, structured hover. Landing feature cards: same accent top-border + sharper radius, h3 weight 700 / -0.015em. Removed stale `glow`/`float` keyframes + `.animate-glow`/`.animate-float` classes from global.css. Landing subtitle copy trimmed. Hero gap tightened. Verified via Playwright on desktop + mobile. | src/styles/variables.css, src/styles/components.css, src/styles/global.css, src/pages/LandingPage.ts, index.html |
...
34. ✅ **Structural design overhaul** — typography hierarchy (800-weight headlines, -0.04em tracking), border-radius tightened (4→14px scale), hazy card shadows removed, accent top-border strips on feature cards, eyebrow badge on landing hero, dead glow element removed. Verified via Playwright.
